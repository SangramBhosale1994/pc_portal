...
//echo "continued";
		...
		$contestant_country = mysqli_real_escape_string($db_connection, $entry_info[10]);
	...
	$group_leaderboard_result = mysqli_query($db_connection, "SELECT * FROM ally_challenge_scores WHERE company_name != '' AND contestant_score>0 ORDER BY contestant_score DESC LIMIT 50");
...
echo "ERROR!3";
	...
	function csvstring_to_array($string, $separatorChar = ',', $enclosureChar = '"', $newlineChar = "\n") {
	    ...
	    /* Only these fields are needed-
			...
			7	"Details"
			11	"When"
			10	"Country (User Details)"
		*/
	    $needed_columns_array = [0, 3, 4, 5, 6, 7, 11, 10];
 	...
 	<div>
		...
		<center>
			...
			<?php
 			...
 			<?php
 				$company_referral_emails = mysqli_query( $db_connection, "SELECT * FROM ally_challenge_scores WHERE reference_email ='".$main_row['contestant_email']."' ORDER BY contestant_score DESC LIMIT 50");
