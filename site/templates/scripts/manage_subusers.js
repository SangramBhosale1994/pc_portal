...
$(document).ready(function () {
    ...
    /* When Add profile Form is submitted */
    $(document).on("click", "#btn_add_subuser", function (event) {
        /* Prevent submitting automatically*/
        event.preventDefault();
        event.stopPropagation();

        /* Serialize the form data to send */
        let add_subuser_form_details = $("#add_subuser_modal_form").serializeArray();

        /* Add the page ID to be changed */
        // add_subuser_form_details.push({
        //     "name": "id",
        //     "value": profile_to_edit.id
        // })

        add_subuser_form_details = JSON.stringify(add_subuser_form_details);
        
        console.log(add_subuser_form_details);

        $.ajax({
            url: homepath + 'api/manage-subusers/',
            method: "POST",
            //dataType : 'text',
            data: {
                "requested_action_to_take": "add_subuser",
                "add_subuser_form_details": add_subuser_form_details
            },
            beforeSend: function () {
                $("#btn_add_subuser").attr('disabled', 'true');
                $("#btn_add_subuser").html('Saving...');
               // $("#btn_add_subuser").hide()
            },
            complete: function () {
                $("#btn_add_subuser").removeAttr("disabled");
                $("#btn_add_subuser").html('Submit');
            },
            success: function (result) {
                console.log(result)
                if (result.error.number_of_errors > 0) {
                    ...
                    // /* Hide the modal */
                    // $("#add_subuser_modal").modal('hide');

                    /* Show the success message */
                    alert(result.success.success_message);

                    /* Refresh the datatable */
                    api_fetch_manage_subusers_request_table_data();
                }
            },
            error: function (er, er2, er3) {
                console.log(er3)
    ...
    $(document).on("click", ".unlock_button", function () {
        ...
        let requested_universal_company_profile_to_unlock = $(this).attr("id").replace("_unlock_button", "");
        console.log(requested_universal_company_profile_to_unlock);
        ...
        $.ajax({
           ...
           // dataType: 'text',
            data: {
                ...
                "requested_universal_company_profile_to_unlock": requested_universal_company_profile_to_unlock
    ...
    $(document).on("click", ".disable_button", function () {
        /* Array of IDs of profiles to be deleted, converted to JSON */
        let requested_sub_user_profile_to_disable = $(this).attr("id").replace("_disable_button", "");
        console.log(requested_sub_user_profile_to_disable);
        ...
        $.ajax({
            ...
            //dataType: 'text',
            data: {
                "requested_action_to_take": "disable_profiles",
                "requested_sub_user_profile_to_disable": requested_sub_user_profile_to_disable
    ...
    function api_fetch_manage_subusers_request_table_data() {
        $.ajax({
            url: homepath + 'api/manage-subusers/',
            method: "POST",
           //dataType: 'text',
            data: {
                "requested_action_to_take": "fetch_manage_subusers_request_table_data"
            },
            beforeSend: function () {
