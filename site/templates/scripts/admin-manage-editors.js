	...
	$(document).ready(function() {
		...
		$(document).on("change", ".editor_checkbox", function () {
...
console.log(new_editor_form_details);
			$.ajax({
...
console.log(er3)
		...
		$(document).on("click", ".profile_edit_button", function(){
			//$("#edit_profile_modal_form")[0].reset();
			profile_to_edit = {};
			...
			for (i = 0; i < all_editors_data.data.length; i++) {
				if(all_editors_data.data[i].id == profile_to_edit.id){
						profile_to_edit=all_editors_data.data[i];
				}
		}
			
			/*select radio button function is used for get name and selected value and check value checked input field */
			...
			$("#edit_profile_editor_unlock_quota").val(profile_to_edit.unlock_quota);
			// $("#edit_profile_editor_accessible_sections_").val(profile_to_edit.accessible_sections_array);
			// select_radio_button("accessible_sections",profile_to_edit.accessible_sections_array);
...
console.log(er3)
		...
		$(document).on("click", "#delete_selected_modal_submit", function(){
			...
			let requested_profiles_to_delete_json = JSON.stringify( $(".editor_checkbox:checked").map(function(){
...
//dataType : 'text',
				data : {
					"requested_action_to_take" : "delete_editors",
...
console.log(er3);
		...
		function api_fetch_editor_table_data(){
			$.ajax({
				url : homepath+'api/admin-manage-editors/',
				method : "POST",
//dataType : 'text',
				data : {
					"requested_action_to_take" : "fetch_editor_table_data"
				},
				beforeSend: function(){
