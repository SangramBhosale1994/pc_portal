      ...
      $("#edit_profile_job_id").val(profile_to_edit.id);
		...
		/* decodeHtml(received_text) function for handling escape characters*/
			...
			if (profile_to_edit.is_show_popup === "Show") {
				...
				$('input[name="edit_job_show_popup"]').prop('checked', true);
			} else {
					// Set the checkbox to unchecked
					$('input[name="edit_job_show_popup"]').prop('checked', false);
			}

			// console.log($("#edit_profile_show_job_popup").val(profile_to_edit.is_show_popup));
...
// 		}
		...
		function fetch_columns(){
			...
			$.ajax({
					url : homepath+'api/admin-manage-jobs/?'+url_parameters,
