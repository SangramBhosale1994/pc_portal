...
// console.log(user_designation);
	...
	/* Phone Number Typing Limitations End */
		/* Code by Amruta allow point and number only in current ctc field */
//     $("#current_ctc").on("keypress", function (event) {
// 		var regex = new RegExp("^\d\d+\.[0-9]\d*$");
// 		var key = String.fromCharCode(!event.charCode ? event.which : event.charCode);
...
// 	})
	...
	/* CTC Typing Preventions ^[0-9]+$   ^[1-9]\d*(?:\.\d{0,2})?$*/
// 	$("#current_ctc").on("keypress", function (event) {
...
// 	})
    ...
    /*End Code by Amruta allow point and number only in current ctc field*/
...
console.log("abc");
		e.preventDefault();
		e.stopPropagation();
		check_is_admin = is_admin;
		check_is_editor = is_editor;
		console.log("is_admin");
		console.log(check_is_admin);
			if(is_admin!="admin" && is_editor!="editor"){
		...
		if ($(this).closest('form').find('input, select, textbox').filter(function () { return this.checkValidity() === false }).length > 0) {
		    console.log($(this).closest('form').find('input, select, textbox').filter(function () { return this.checkValidity() === false }) );
		    console.log(true);
		    //alert("true");
...
console.log("1");
	...
	$(document).on("submit", "#skills-array-modal-form", function (skills) {
		// $(".btn-submit-skills").on('click', function (skills) {
	...
	/* Redacted Resume Upload Extension Check */
	$(function () {
		$('#redacted_resume').change(function () {
			let val = $(this).val().toLowerCase();
			let regex = new RegExp("(.*?)\.(docx|doc|pdf)$");

			if (!(regex.test(val))) {
				$(this).val('');
				$("#redacted_resume_label").html("PDF or MS Word files only");
				alert('Please upload a .docx, .doc or .pdf file.');
			} else {
				$("#redacted_resume_label").html($(this).val().split("\\").pop());
			}
		});
	});
	/* Redacted Resume Upload Extension Check End */
...
/**  in function pass technical_skills_array, non_technical_skills_array_soft_skills_array */
function skill_confirm_modal(technical_skills_array, non_technical_skills_array, soft_skills_array) {
	// console.log("2");
	/** here show or display modal of upload resume skills and click on submit button */
	...
	for (let value of non_technical_skills_array) {
		...
		/** display checkbox and non technical skills array value */
		/** html imput assign to checkbox variable */
	...
	for (let value of soft_skills_array) {
		...
		/** display checkbox and soft skills array value */
		/** html imput assign to checkbox variable */
	...
	// var skills_checked_array = [];
	// $.each($("checkbox:checked"), function () {
	// 	skills_checked_array.push($(this).val());
	// });
}
