...
$(document).ready(function() {
    ...
    /* getting the URL parameters in case there are pre-set filters */
    let url_string = window.location.href; //window.location.href
    var url = new URL(url_string);

    /* searching parameters */
    if(url.searchParams.get("fresher")==="true"){
        $("#experience_years").val("0");
        url.searchParams.delete("fresher");
    }
    
    if(url.searchParams.get("internship")==="true"){
        $("#job_type").val("Internship");
    ...
    if(url.searchParams.get("hot_jobs")==="true"){
        $("#job_type").val("Hot Jobs");
        url.searchParams.delete("hot_jobs");
    }

    if(url.searchParams.get("jobs_searchword")!=""){
        $("#jobs_searchword").val(url.searchParams.get("jobs_searchword"));
        url.searchParams.delete("jobs_searchword");
    }
    if(url.searchParams.get("experience_years")!=""){
        $("#experience_years").val(url.searchParams.get("experience_years"));
        url.searchParams.delete("experience_years");
    }
    
    if(url.searchParams.get("preferred_location")!=""){
        $("#preferred_location").val(url.searchParams.get("preferred_location"));
        url.searchParams.delete("preferred_location");
    }
...
/**Job data fetching api  */
    function fetch_jobs_data(start,search_form_details,quick_search_details){
        ...
        $("#job_container_row").html("<h4 class='text-center m-auto pb-2'>Loading...</h4>");
        
        var url_string = window.location.href; //window.location.href
        var url = new URL(url_string);
        var get_company_id = url.searchParams.get("company_id");
        console.log(get_company_id);
        if(get_company_id==null){
            get_company_id=false;
        ...
        /* searching parameters */
        let is_hot_jobs = false;
        if(url.searchParams.get("hot_jobs")==="true"){
            is_hot_jobs=true;
             url.searchParams.delete("hot_jobs");
        }
        let is_other_jobs = false;
        if(url.searchParams.get("other_jobs")==="true"){
            is_other_jobs=true;
        }
...
//   dataType : 'text',
            ...
            data : {
                ...
                "is_hot_jobs" : is_hot_jobs
            },
        
            success : function(result){
                $("#job_container_row").html("");	
                /** JSON.stringify() is used for convert object to string*/
            //    var result= JSON.stringify(result);
             /** convert json string to object using JSON.parse() function and assign to variable*/
    ...
    // console.log(result1);
            ...
            //    }
                ...
                if(result.error.number_of_errors > 0){
                    console.log("No data");
                    $("#job_container_row").append('<div class="alert alert-danger m-auto mb-1" role="alert" style="padding:0.5rem 1.25rem;"><strong>'+result.error.error_message+'</strong></div>');
                    total_cards=result.recordsTotal;
                    total_record=result.recordsTotal;
            ...
            error : function(){
                $("#job_container_row").html("");
                $("#job_container_row").append('<div class="alert alert-danger m-auto mb-2" role="alert" style="padding:0.5rem 1.25rem;"><strong>Something went wrong, please refresh the page and try again. </strong></div>');
            }
        });
    }
    ...
    search_form_details = $("#search-container").serializeArray();
    quick_search_details = $("#jobs_searchword").val();
   ...
   // console.log("total rescord count");
    ...
    function add_card(arr_result){
       ...
       // console.log("************");
        var card=` <div class="( job-card-container )[ my-3 ][ ${arr_result.counter} ][ col-md-6  col-lg-4 col-xl-4 ]" style="<?=$is_visible?>">
                        ...
                        <div class="[ card ]">`;
                         if(arr_result['hot_jobs'] =="yes"){
                            card+=`<span class="[ px-3 ][ badge badge-danger ]" style="position: absolute; right:41%; top:-0.6rem;  font-size: 0.9rem; font-weight: 600; padding-bottom: 0.3rem">HOT</span>`;
                        }
                        else if(arr_result['new'] !=""){
                            ...
                            <div class="card-body textcolor job_card_body">
                                ...
                                <div class="container pt-1 m-0">
                                    ...
                                    <div class="row align-items-center" style="   height:98px;overflow: hidden ">
                                        ...
                                        ?>
                                        <a href="${arr_result.company_profile_link}">
   ...
   $("#btn_quick_search").on('click', function (e) { 
    ...
    /* End Quick search */
    /**Quick search */
   $("#btn_quick_search_mobile").on('click', function (e) { 
    // event.preventDefault();
    event.stopPropagation();
    /* Serialize the input data to send */
    let quick_search_details = $("#jobs_searchword_mobile").val();
...
//     });
	...
	/** Advance search when search form load */
    ...
    /***Mobile view Advance search */
    $("#btn_search_submit_mobile").on('click', function (e) { 
        //$("#search-modal").show();
        /* Prevent submitting automatically*/
        // event.preventDefault();
        event.stopPropagation();
       ...
       // console.log("form submit");
        /* Serialize the form data to send */
        // let search_form_details = JSON.stringify($("#search-container").serializeArray());
        let search_form_details = $("#search-container-mobile").serializeArray();
        /* Save the data to send to the AJAX page into a variable */
        // console.log(search_form_details);
        // console.log("------");
        let quick_search_details = $("#jobs_searchword").val();
