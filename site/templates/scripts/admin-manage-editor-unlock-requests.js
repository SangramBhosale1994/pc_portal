...
console.log(er3)
		...
		$(document).on("click", ".unlock_button", function(){
			...
			let requested_editor_profile_to_unlock = $(this).attr("id").replace("_unlock_button", "");

			/* Ajax request to delete the selected profile */
			$.ajax({
				url : homepath+'api/admin-manage-editor-unlock-requests/',
				method : "POST",
//dataType : 'text',
				data : {
					"requested_action_to_take" : "unlock_profiles",
					"requested_editor_profile_to_unlock" : requested_editor_profile_to_unlock
				},
				beforeSend: function(){
...
//dataType : 'text',
				data : {
					"requested_action_to_take" : "fetch_editor_unlock_request_table_data"
				},
				beforeSend: function(){
