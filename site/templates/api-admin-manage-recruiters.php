...
// }
	...
	function fetch_recruiter_table_data(){
		...
		/* Loop through all the recruiter profile pages */
		 $serial_counter=1;
		 $super_recruiter=wire('pages')->get("name=recruiters");
		 
		foreach(wire('pages')->get("name=recruiters")->children("sort=published") as $recruiter_page) {
...
// 			}
			...
			if($recruiter_page->total_unlock_quota!=""){
				$recruiter_data->total_unlock_quota = $recruiter_page->total_unlock_quota;
			}
			else{
				$recruiter_data->total_unlock_quota = "0";
			}
			if($recruiter_page->available_total_unlock_quota!=""){
				$recruiter_data->remaining_total_unlock_quota = $recruiter_page->available_total_unlock_quota;
			}
			else{
				$recruiter_data->remaining_total_unlock_quota = "0";
			}
			...
			if($recruiter_page->applicants_unlock_quota!=0){
				$recruiter_data->unlocked_profiles_count=($unlocked_profiles_count-((int)$recruiter_page->applicant_unlock_counter));
				// echo "if";
			}
			else{
			...
			// echo $recruiter_data->unlocked_profiles_count;
			// echo "----";
			...
			if($recruiter_page->applicant_unlock_counter!=0){
				if(((int)$recruiter_page->applicants_unlock_quota-(int)$recruiter_page->applicant_unlock_counter)>0){
					$recruiter_data->applicants_unlocked_profiles_count = $recruiter_page->applicant_unlock_counter;
				}
				else{
			...
			$serial_counter++;
			$job_posted_count=0;
			$job_posted_count=wire('pages')->get("name=jobs")->children("job_added_by=".$recruiter_page->id)->count();
			// echo "----";
			...
			foreach($recruiter_page->children() as $single_page){
				$sub_recruiter_job_posted_count=wire('pages')->get("name=jobs")->children("job_added_by=".$single_page->id)->count();
      ...
      $recruiter_data->job_posted_count=$job_posted_count;
			// echo $job_posted_count;
				...
				if($recruiter_page->job_added_by!=0){
					...
					$admin_page=wire('pages')->get("name=admins")->child("id=".$recruiter_page->job_added_by);
					//echo $admin_page->company_name;
					$editor_page=wire('pages')->get("name=editors")->child("id=".$recruiter_page->job_added_by);
			...
			// $posted_jobs_link=wire('pages')->get("name=admin-manage-jobs")->httpUrl."?job_added_by=".$recruiter_page->id;
			$posted_jobs_link=wire('config')->urls->httpRoot."resume/pc-admin/admin-manage-jobs/?job_added_by=".$recruiter_page->id;
			...
			$sub_recruiter_data_link=wire('pages')->get('name=recruiter-manage-sub-recruiter')->httpUrl."?user_id=".$recruiter_page->id;
		...
		$to_return->columns = [
			...
			["title"=>"Total Unlock Quota", "data"=>"total_unlock_quota"],
			["title"=>"Remaining Total Unlock Quota", "data"=>"remaining_total_unlock_quota"],
	...
	function delete_recruiters($requested_profiles_to_delete_json){
		...
		foreach ($requested_profiles_to_delete as $profile_to_delete_id) {
			if(wire("pages")->get("name=recruiters")->child("id=".$profile_to_delete_id)->trash()){
	...
	function modify_recruiter($edit_profile_form_details){
		...
		$recruiter_page_to_change = wire('pages')->get($edit_recruiter_details_to_save['id']);
		...
		// echo $edit_recruiter_details_to_save['recruiter_total_unlock_quota'];
		...
		$recruiter_page_to_change->total_unlock_quota = $edit_recruiter_details_to_save['recruiter_total_unlock_quota'];
		$recruiter_page_to_change->available_total_unlock_quota = $edit_recruiter_details_to_save['recruiter_total_unlock_quota'];
		$recruiter_page_to_change->unlock_quota = $edit_recruiter_details_to_save['recruiter_unlock_quota'];
		$recruiter_page_to_change->main_unlock_quota = $edit_recruiter_details_to_save['recruiter_unlock_quota'];
		...
		$recruiter_page_to_change->recruiter_profile_type = "super-recruiter";
		$recruiter_page_to_change->recruiter_accessible_sections =PageArray($accessible_sections) ;
	...
	function create_recruiter($new_recruiter_form_details){
		...
		$new_recruiter_page = wire(new Page());
		$new_recruiter_page->template = wire('templates')->get("recruiter-profile");
		$new_recruiter_page->parent = wire('pages')->get("name=recruiters");
		...
		$single_recruiter=wire('pages')->get("name=recruiters")->child("email=".$new_recruiter_details_to_save['recruiter_email']);
		...
		$new_recruiter_page->total_unlock_quota = $new_recruiter_details_to_save['recruiter_total_unlock_quota'];
		$new_recruiter_page->available_total_unlock_quota = $new_recruiter_details_to_save['recruiter_total_unlock_quota'];
		$new_recruiter_page->unlock_quota = $new_recruiter_details_to_save['recruiter_unlock_quota'];
		$new_recruiter_page->main_unlock_quota = $new_recruiter_details_to_save['recruiter_unlock_quota'];
		...
		$new_recruiter_page->job_added_by = wire('session')->user_page_id;
