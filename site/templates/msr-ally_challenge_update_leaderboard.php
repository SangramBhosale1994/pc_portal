...
error_reporting(E_ALL);
	...
	foreach ($array_for_calculation as $entry_info) {
		...
		if(mysqli_num_rows($check_account_query) === 1){
			...
			if($account_info["profile_status"] === "incomplete"){
				...
				mysqli_query($db_connection, "UPDATE `ally_challenge_scores` SET `contestant_name`='$contestant_name',`profile_status`= 'complete' WHERE contestant_email='$contestant_email'");
			}
		...
		/* If account does not exist */
			...
			add_contestant($db_connection, $contestant_email, 0, $contestant_name, "incomplete");
		}
		...
		// 	if($entry_action === "Refer & Earn Bonus Entries (Points) #WinTogether"){
	...
	// $group_leaderboard_result = mysqli_query($db_connection, "SELECT * FROM ally_challenge_scores WHERE company_name != '' AND contestant_score>0 ORDER BY contestant_score DESC LIMIT 100");
	...
	$individual_leaderboard_result = mysqli_query($db_connection, "SELECT * FROM ally_challenge_scores WHERE company_name = '' ORDER BY contestant_score DESC LIMIT 50");
...
//echo $dataArray_json;
	...
	function add_contestant($db_connection, $contestant_email, $entry_score, $contestant_name, $profile_status){
		$add_contestant_result = mysqli_query($db_connection, "INSERT INTO ally_challenge_scores (`contestant_email`, `contestant_score`, `contestant_name`, `profile_status`) VALUES ('$contestant_email',$entry_score,'$contestant_name', '$profile_status')");
...
echo "ERROR!3";
	...
	function csvstring_to_array($string, $separatorChar = ',', $enclosureChar = '"', $newlineChar = "\n") {
	    ...
	    /* Only these fields are needed-
			...
			11	"When"
		*/
	    $needed_columns_array = [0, 3, 4, 5, 6, 7, 11];
