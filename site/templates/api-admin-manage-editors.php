...
if(!isset($_SERVER["HTTPS"]) || $_SERVER["HTTPS"] != "on")
{
	...
	function fetch_editor_table_data(){
		...
		foreach(wire('pages')->get("name=editors")->children("sort=-published") as $editor_page) {
	...
	function delete_editors($requested_profiles_to_delete_json){
		...
		foreach ($requested_profiles_to_delete as $profile_to_delete_id) {
			if(wire("pages")->get("name=editors")->child("id=".$profile_to_delete_id)->trash()){
...
//print_r($edit_profile_form_details);
		...
		$editor_page_to_change = wire('pages')->get($edit_editor_details_to_save['id']);
		...
		$editor_page_to_change->accessible_sections =PageArray($accessible_sections) ;
	...
	function create_editor($new_editor_form_details){
		...
		$new_editor_page = wire(new Page());
		$new_editor_page->template = wire('templates')->get("editor-profile");
		$new_editor_page->parent = wire('pages')->get("name=editors");
