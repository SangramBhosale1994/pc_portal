...
// }
	...
	function fetch_sub_recruiter_table_data(){
      ...
      $to_return->user_id=$_GET['user_id'];
			...
			wire("session")->recruiter_page_id=$recruiter_page_id;
    }
		else{
		...
		$recruiter_page=wire('pages')->get($session_user_id);
		// if($filter_selector!=""){
		// 	$super_recruiter_page=wire('pages')->get($session_user_id,$filter_selector);
		// 	$to_return->recruiter_page=wire('pages')->get($session_user_id,$filter_selector);
		...
		// 	}
		// 	foreach(wire('pages')->get($session_user_id)->children($filter_selector) as $sub_recruiter_page) {
		// 		$recruiters_data_array[]=$sub_recruiter_page->id;
		// 	}
		// }
		// else{
			$recruiters_data_array[]=$recruiter_page->id;
			foreach(wire('pages')->get($session_user_id)->children("sort=published") as $sub_recruiter_page) {
				$recruiters_data_array[]=$sub_recruiter_page->id;
			}
		// }
		...
		foreach($recruiters_data_array as $sub_recruiter_page_id) {
			$sub_recruiter_page=wire('pages')->get($sub_recruiter_page_id);
			...
			if($search_from_date!="" && $search_to_date!=""){
				...
				$counter=0;
				foreach(wire('pages')->get("name=jobs")->children("job_added_by=".$sub_recruiter_page->id) as $single_job_page){
				...
				/**Calculate count of jobs posted by each sub-recruiter */
				$sub_recruiter_data->job_posted_by_sub_recruiter=wire('pages')->get("name=jobs")->children("job_added_by=".$sub_recruiter_page->id)->count();
			}
			// echo "++++";
			...
			foreach(wire('pages')->get("name=candidates")->children("recruiter_candidate_status!=") as $candiate_page){
			...
			$posted_jobs_link=wire('pages')->get("name=recruiter-manage-jobs")->httpUrl."?job_added_by=".$sub_recruiter_page->id;
			...
			$report_page_link=wire('pages')->get("name=recruiter-report-page")->httpUrl."?user_id=".$sub_recruiter_page->id;
	...
	function delete_sub_recruiters($requested_profiles_to_delete_json){
		...
		foreach ($requested_profiles_to_delete as $profile_to_delete_id) {
			if(wire("pages")->get($session_user_id)->child("id=".$profile_to_delete_id)->trash()){
	...
	function modify_sub_recruiter($edit_profile_form_details){
		...
		$sub_recruiter_page_to_change = wire('pages')->get($edit_sub_recruiter_details_to_save['id']);
	...
	function create_sub_recruiter($new_sub_recruiter_form_details){
		...
		$new_sub_recruiter_page = wire(new Page());
		$new_sub_recruiter_page->template = wire('templates')->get("recruiter-profile");
		$new_sub_recruiter_page->parent = wire('pages')->get($session_user_id);
		...
		$single_recruiter=wire('pages')->get($session_user_id)->child("email=".$new_sub_recruiter_details_to_save['sub_recruiter_email']);
		...
		// $new_sub_recruiter_page->job_added_by = wire('session')->user_page_id;
		$new_sub_recruiter_page->company_name = wire('session')->company_name;
