...
// 	 print_r($company_admin_profile);
		...
		$serial_counter=1;
		foreach(wire('pages')->get("name=seller-profiles")->children("sort=published") as $seller_page) {
		  ...
		  //  foreach(wire('pages')->get("name=universal-profiles")->children("id=".$seller_page->seller_profile_id) as $seller_admin_profile){
		    $seller_admin_profile= wire('pages')->get("name=universal-profiles")->child("id=".$seller_page->seller_profile_id);   
		  //  print_r($company_admin_profile);
		  //  echo "1";
...
// 			$seller_data->unlock_button = "<button id='".$company_page->id."_unlock_button' class='[ btn btn-primary ]( unlock_button )' type='button'><i class='[ mr-2 ][ fas fa-unlock ]'></i>Verify</button>";
			...
			//$seller_data->marking_button = "<a target='_blank' href='".wire('pages')->get("name=admin-marking")->httpUrl."?page_to_score=".$company_page->id."' id='".$company_page->id."_marking_button' class='[ btn btn-primary ]( marking_button )' type='button'><i class='[ mr-2 ][ fas fa-check ]'></i>Show</a>";
...
// 			["title"=>"Employees", "data"=>"employee_count"],
	...
	function unlock_profiles($requested_seller_profiles_to_unlock){
		...
		$company_page = wire("pages")->get("name=seller-profiles")->child("id=".$requested_seller_profiles_to_unlock);
...
// 		$link = "Greetings from Pride Circle!\n\nThe OTP for your login is ".<a href="'.$a.'">".$otp."</a>";
	...
	function unverify_profiles($requested_referral_profile_to_unverify_profiles,$seller_rejection_reason){
		...
		/* Get the page of the editor */
		$seller_page = wire("pages")->get("name=seller-profiles")->child("id=".$requested_referral_profile_to_unverify_profiles);
...
//echo "2";
        //$seller_rejection_reason=wire('input')->post->text('rejection_reason');
...
// 		$subject = "Activate your account: Pride Circle Corporate Dashboard";
		...
		$url =wire("pages")->get("name=universe")->httpUrl;
			...
			$seller_profile_page = wire("pages")->get("name=universal-profiles")->child("seller_page_id=".$seller_page->id);
...
// echo "--------------------------";
// print_r( wire("pages")->get("name=seller-profiles")->child("id=".$profile_to_delete_id));
			if(wire("pages")->get("name=seller-profiles")->child("id=".$profile_to_delete_id)->trash()){

				//$successfully_deleted_profiles[] = $profile_to_delete_id;
				if(wire("pages")->get("name=universal-profiles")->child("seller_page_id=".$profile_to_delete_id)->trash()){
