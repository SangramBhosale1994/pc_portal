...
// 			}
	...
	function unlock_profiles($requested_universal_company_profile_to_unlock){
		...
		//$company_page = wire("pages")->get("name=universal-companies")->child("id=".$requested_universal_company_profile_to_unlock);
...
// 		if($input->post->email){
        ...
        // 	   else
    		...
    		$new_profile_page->template = wire('templates')->get("unregistered_profiles");
    		$new_profile_page->parent = wire('pages')->get("name=unregistered-profiles");
    		...
    		$new_profile_page->refered_by = wire('session')->email;
	 ...
	 // print_r($sub_users);
      ...
      $company_page->save();
		...
		$np = new Page();
		$np->template = wire('templates')->get("otp_verification");
		$np->parent = wire('pages')->get("name=user-registration-with-email");
		$np->title = $otp;
		$np->email = $email;
		$np->otp_status = "live";
		$np->of(false);
		...
		$company_admin=wire('pages')->get("name=universal-profiles")->child('company_name='.$company_name);
		$admin_name=$company_admin->title;
		//$a ="http://zerovaega.in/pc_rportal/universe/user-registration-with-email/?otp=$otp&email=$email";
		$a =wire('pages')->get("name=user-registration-with-email")->httpUrl."?otp=$otp&email=$email";
