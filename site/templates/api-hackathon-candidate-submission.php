   ...
   foreach ($all_candidates_array as $candidate_page) {
      ...
      //  $candidate_count++;
		...
		if($session->user_designation == "admin" || $session->user_designation == "editor") 
		{
			...
			if($candidate_page->hackathon_files){
				$candidate_data->active_button = ""; // Initialize the variable
				foreach($candidate_page->hackathon_files as $single_file){
					$candidate_data->active_button .="<a target='_blank' href='".$single_file->httpUrl."' id='".$candidate_page->id."' class='[   ]' type='' data-toggle='tooltip' data-placement='top' title='Submission File'>$single_file->name</a><br>";
					
					
				}
				$candidate_data->view_files=$candidate_data->active_button;
				
			}
			else{
				$candidate_data->view_files="-";
			}
...
// 			$company_data->registration_time=date('d/m/Y h:i A', $company_data->registration_time);
  ...
  $view_submissions = new stdClass();
	$view_submissions->title = "View Submissions";
	$view_submissions->data = "view_files";
  ...
  $to_return->columns[] = $view_submissions;
