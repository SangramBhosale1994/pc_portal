...
// 		break;
	...
	function fetch_job_table_data(){
		...
		if(wire("session")->recruiter_page_id!=""){
			$session_page_id=wire("session")->recruiter_page_id;
		}
		else{
			$session_page_id=wire("session")->user_page_id;
		}
		if(wire("session")->recruiter_profile_type == "sub-recruiter"){
			$recruiter_page = wire("pages")->get("id=".$session_page_id)->parent();
			...
			$recruiter_page = wire("pages")->get("id=".$session_page_id);
			$recruiter_page_array[].=$recruiter_page->id;
			foreach(wire("pages")->get("id=".$session_page_id)->children("recruiter_profile_type=sub-recruiter") as $sub_recruiter_page){
				$recruiter_page_array[].=$sub_recruiter_page->id;
			}
		...
		foreach($recruiter_page_array as $recruiter_page_id){
			...
			foreach(wire('pages')->get("name=jobs")->children("job_added_by=".$recruiter_page_id) as $job_page) {
	...
	// 			$job_data->job_publish_shedule = date("d M Y", intval($job_page->job_publish_shedule));
			...
			//	$job_data->job_publish_shedule= date("d M Y", $job_page->job_publish_shedule);
				...
				$applicants_list_link=wire('pages')->get("name=recruiter-candidate-table")->httpUrl."?job_code=".$job_data->job_code;
...
// 		$recruiter_page = wire("pages")->get("name=recruiters")->child("id=".$user_page_id);
// 		//echo $recruiter_page;
...
// 	}
    ...
    //echo "++++++++++++";
		...
		$job_page = wire("pages")->get("id=".$requested_job_profile_to_active);
// 		echo $job_page->id;
	...
	function inactive_profiles($requested_job_profile_to_inactive_profiles){
		...
		/* Get the page of the editor */
		$job_page = wire("pages")->get("id=".$requested_job_profile_to_inactive_profiles);
		$job_page->of(false);
//echo "1";
