...
$to_return->enda =Array();
		...
		foreach(wire('pages')->get("name=workshops")->children() as $event_page) {
...
//$to_return->enda[] = (int)$event_page->event_end_date;
			...
			if($event_page->job_added_by !=0){
				...
				$admin_page=wire('pages')->get("name=admins")->child("id=".$event_page->job_added_by);
				//echo $admin_page->company_name;
				$editor_page=wire('pages')->get("name=editors")->child("id=".$event_page->job_added_by);
				//echo $recruiter_page;
			...
			//echo wire('session')->user_designation;
			if(wire('session')->user_designation=="admin"){
				$applicants_list_link=wire('pages')->get("name=admin-candidate-table")->httpUrl."?event_code=".$event_data->event_code;
			}
			elseif(wire('session')->user_designation=="editor"){
				$applicants_list_link=wire('pages')->get("name=editor-candidate-table")->httpUrl."?event_code=".$event_data->event_code;
			}
			else{
	...
	function modify_event($edit_profile_form_details){
		...
		$event_page_to_change = wire('pages')->get($edit_event_details_to_save['id']);
	...
	function create_event($new_event_form_details){
		...
		$new_event_page->job_added_by = wire('session')->user_page_id;
		...
		$event_code_serial_id_counter_page = wire('pages')->get("name=event-code-serial-id-counter");
