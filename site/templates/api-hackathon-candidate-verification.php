...
//  $candidate_count=0;
 ...
 $search_candidate_id=array();
  $all_candidates_array=new PageArray();    
	// $search_counter=1;
		...
		// echo $filter_selector;
		// echo "---";
		 $all_candidates_array=$pages->get("name=candidates")->children("hackathon_interested=Yes,".$filter_selector);
   ...
   foreach ($all_candidates_array as $candidate_page) {
      ...
      //  $candidate_count++;
		...
		// $candidate_data->active_button = "<button id='".$candidate_page->id."_active_button' class='[ btn btn-primary ]( active_button )' type='button'><i class='[ mr-2 ][ fas fa-unlock ]'></i>Activate</button>";
		$candidate_data->verify_button = "<button id='".$candidate_page->id."_verify_button' class='[ btn btn-outline-primary ]( verify_button )' type='button'><i class='[ mr-2 ][ fas fa-unlock ]'></i>Verify</button>";
		$candidate_data->unverify_button = "<button id='".$candidate_page->id."_unverify_button' class='[ btn btn-outline-danger ]( unverify_button )' type='button'><i class='[ mr-2 ][ fas fa-lock ]'></i>Unverify</button>";
		
		if($candidate_page->hackathon_verification =="verified" && $candidate_page->hackathon_verification !=""){
			$candidate_data->verify_action="-";
			$candidate_data->unverify_action=$candidate_data->unverify_button ;
		}
		else{
		  $candidate_data->verify_action=$candidate_data->verify_button ;
		}

		if($candidate_page->hackathon_verification =="unverified" && $candidate_page->hackathon_verification !=""){
			$candidate_data->unverify_action="-";
			$candidate_data->verify_action=$candidate_data->verify_button ;
		}
		else{
		  $candidate_data->unverify_action=$candidate_data->unverify_button ;
		...
		if($session->user_designation == "admin" || $session->user_designation == "editor") 
		{
			...
			// $candidate_data->verify_button ="<a target='_blank' href='".$candidate_page->httpUrl."' id='".$candidate_page->id."_verify_button' class='[ btn btn-outline-primary  ]' type='button' data-toggle='tooltip' data-placement='top' title='Verify Profile'>Verify</a>";
		...
		foreach ($columns_to_show as $column_id => $column_title) {
          ...
          case "ID":
            $candidate_data->id = $candidate_page->id;
            break;
...
// 			$company_data->registration_time=date('d/m/Y h:i A', $company_data->registration_time);
  ...
  $verify_profile = new stdClass();
	$verify_profile->title = "Verify";
	$verify_profile->data = "verify_action";

	$unverify_profile = new stdClass();
	$unverify_profile->title = "Unverify";
	$unverify_profile->data = "unverify_action";
  ...
  $to_return->columns[] = $verify_profile;
	$to_return->columns[] = $unverify_profile;
