...
//$universal_profiles_page=wire('pages')->get('name=universal-profiles');
        $serial_counter=1;
		foreach(wire('pages')->get("name=universal-companies")->children("sort=published") as $company_page) {
			//$company_page_title=$sanitizer->selectorValue($company_page->title);
		    $company_admin_profile = wire('pages')->get("name=universal-profiles")->child("company_page_id=".$company_page->id);
