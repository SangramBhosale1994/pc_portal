    ...
    if(isset($_GET['user_id'])){
      ...
      $to_return->user_id=$_GET['user_id'];
		...
		if(wire("session")->recruiter_profile_type == "sub-recruiter"){
			$super_recruiter_page = wire("pages")->get("id=".$session_page_id)->parent();
			...
			$super_recruiter_page = wire("pages")->get("id=".$session_page_id);
			$recruiter_page_array[].=$super_recruiter_page->id;
        ...
        $candidate_email="";
				if(!empty($candidate_array)){
					foreach($candidate_array as $single_candidate){
						...
						$candidate_page=wire("pages")->get("name=candidates")->child("id=".$profile_id.",active_status=active");
