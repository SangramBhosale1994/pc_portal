...
if (!defined('K_TCPDF_EXTERNAL_CONFIG') OR !K_TCPDF_EXTERNAL_CONFIG) {
	...
	$tcpdf_config_files = array(dirname('./vendor/tecnickcom/tcpdf/tcpdf_autoconfig.php').'/config/tcpdf_config.php', '/etc/php-tcpdf/tcpdf_config.php', '/etc/tcpdf/tcpdf_config.php', '/etc/tcpdf_config.php');
	foreach ($tcpdf_config_files as $tcpdf_config) {
		if (@file_exists($tcpdf_config) AND is_readable($tcpdf_config)) {
 require_once(\ProcessWire\wire('files')->compile($tcpdf_config,array('includes'=>true,'namespace'=>true,'modules'=>true,'skipIfNamespace'=>true)));
			break;
		}
	}
}
...
if (!defined('K_PATH_MAIN')) {
	define ('K_PATH_MAIN', dirname('./vendor/tecnickcom/tcpdf/tcpdf_autoconfig.php').'/');
}
