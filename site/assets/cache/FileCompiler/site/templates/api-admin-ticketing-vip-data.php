...
//$universal_profiles_page=\ProcessWire\wire('pages')->get('name=universal-profiles');
        $serial_counter=1;
		foreach(\ProcessWire\wire('pages')->get("name=vip-attendee")->children("sort=published") as $ticketing_vip_page) {
			...
			foreach($ticketing_vip_page->children() as $single_event_page)
			{
			   // echo $single_event_page->title;
			    $ticketing_vip_data->event_title_array[]=\ProcessWire\wire('pages')->get('name=tickets-vip')->child('ticketing_event_id='.$single_event_page->title)->title;
			}
			    $ticketing_vip_data->event_title=implode("<br><hr>",$ticketing_vip_data->event_title_array);
			  
			 //   $ticketing_vip_data->event_title=\ProcessWire\wire('pages')->get("name=tickets-vip")->child("ticketing_event_id=".$single_event_page->title)->title;
	...
	function verify_profiles($requested_job_fair_to_verify){
		...
		//echo $requested_job_fair_to_verify;
			...
			$ticketing_vip_page = \ProcessWire\wire("pages")->get("name=vip-attendee")->child("id=".$requested_job_fair_to_verify);
...
//echo"5";
		...
		foreach($ticketing_vip_page->children() as $single_event_page)
			{
			   // echo $single_event_page->title;
			    $event_title_array[]=\ProcessWire\wire('pages')->get('name=tickets-vip')->child('ticketing_event_id='.$single_event_page->title)->title;
			}
