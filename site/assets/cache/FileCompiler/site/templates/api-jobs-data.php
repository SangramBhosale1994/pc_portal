...
// ob_start();
	...
	global $post_values;
	$post_values=$_POST;
	$myfile = fopen($config->paths->templates."includes/newfile_job.txt", "w");

	$to_return->post_values=$post_values;
  ...
  // print_r($post_values);
	...
	/**add datables filters in array */
		...
		//  $start_limit_filter_array[]="limit=185";
		$to_return->limit=$post_length;
		}
	
		/**search filter using advance search */
		if(isset($_POST['is_hot_jobs'])){
			if($_POST['is_hot_jobs']=="true"){
				$search_filter_array[]="is_hot_jobs=yes";
$to_return->search_filter_array = $search_filter_array;
			}
		}
				...
				foreach($search_form_data as $key=>$value){
					...
					if($value['name']=="job_type"){
						if($value['value']!=""){
				            ...
				            /* Check if the job type contains hot jobs */			
							if (in_array("Hot Jobs", $job_type_array)) {
                                $search_filter_array[] = "is_hot_jobs=yes";
                                $job_type_array = array_diff($job_type_array, ["Hot Jobs"]);
                            }
		...
		if(isset($_POST['quick_search_data'])){
			...
			// 	$to_return->match_special_characters=$match_special_characters;
				if(preg_match('[@_!#$%^&*()<>?/|}{~:]', $quick_search_data)){
					...
					$quick_search_data=$special_quick_search_data;
	...
	if(isset($_POST['get_company_id']) && $_POST['get_company_id']!='false'){
		...
		$recruiter_page=$pages->get($_POST['get_company_id']);
		$recruiter_id=Array();
		// echo $recruiter_page->id;
		// print_r($recruiter_page);
		// echo "00";
		if($recruiter_page->recruiter_profile_type=="sub-recruiter"){
			$recruiter_page=$pages->get($_POST['get_company_id'])->parent();
			// echo "11";
		}
		else{
				$recruiter_page=$pages->get($_POST['get_company_id']);
		...
		$recruiter_id[]=$recruiter_page->id;
		foreach($recruiter_page->children() as $single_recruiter){
			$recruiter_id[]=$single_recruiter->id;
		}
		// print_r($recruiter_id);
		// $recruiter_id_array=array();
		// $recruiter_id_array=array_map('trim', array_filter(explode(",",$recruiter_id)));
		// $to_return->recruiter_id_array=$recruiter_id_array;
		$recruiter_id_implode_data=implode("|",$recruiter_id);
		$search_filter_array[]="job_added_by*=".$recruiter_id_implode_data;
...
$to_return->filter_selector_array=$search_filter_array;//debug
  $filter_selector=implode(",",$search_filter_array);
   $to_return->filter_selector=$filter_selector;//debug
	 ...
	 $to_return->start_limit_selector1="0000";
...
//  $filter_selector="job_added_by=10448";
//  //"start_limit_selector":"company_name%=Zerovaega,start=0,limit=100000,limit=24"';
//  $start_limit_selector="job_added_by=10448,start=0,limit=100000,limit=24";
// echo $filter_selector;
// echo "****";

 $all_jobs_array_without_filter=\ProcessWire\wire('pages')->get("name=jobs")->children("sort=-published,verification=verified,active_status=active,".$filter_selector);
//  $all_jobs_array=\ProcessWire\wire('pages')->get("name=jobs")->children("sort=-published,".$filter_selector);
 $all_jobs_array=\ProcessWire\wire('pages')->get("name=jobs")->children("sort=-published,verification=verified,active_status=active,".$start_limit_selector);
...
$to_return->all_jobs_array=$all_jobs_array;
$to_return->all_jobs_array_count=$all_jobs_array->count();
// foreach(\ProcessWire\wire('pages')->get("name=jobs")->children("sort=published") as $job_page) {
	// echo "+++";
	// print_r($all_jobs_array);
	// echo "---";
  ...
  foreach($all_jobs_array as $job_page) {
    ...
    if($job_page->last_modified_date !=""){
      ...
      $job_data->last_modified_date="-";
		...
		/**Create link for company profile of job */
...
// echo "***";		
// echo $company_profile_present;
			if($company_profile_present=="true"){
				$job_data->company_profile_link=\ProcessWire\wire('pages')->get('name=company=profile')->httpUrl."?name=".$job_page->company_name;
			}
			else{
				$job_data->company_profile_link=$job_page->httpUrl;
			}
...
// $myfile = fopen(\ProcessWire\wire('config')->paths->templates."includes/newfile.txt", "w");
// $txt = json_encode($to_return);
// // $txt = json_encode($post_values);
// fwrite($myfile, $txt);
// // fwrite($myfile, "Hello demo");
