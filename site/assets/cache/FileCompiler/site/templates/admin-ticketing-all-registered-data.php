<?php
 require_once(\ProcessWire\wire('files')->compile('includes/admin-header.php',array('includes'=>true,'namespace'=>true,'modules'=>true,'skipIfNamespace'=>true)));
	...
	*/
	...
	foreach($pages->get("name=vip-attendee,status<" . \ProcessWire\Page::statusTrash)->children("sort=-published,verification=verified,status<" . \ProcessWire\Page::statusTrash) as $ticketing_vip_page) {
	...
	*/
	...
	foreach($pages->get("name=attendee,status<" . \ProcessWire\Page::statusTrash)->children("sort=-published,status<" . \ProcessWire\Page::statusTrash) as $ticketing_vip_page) {
            $temp_object=new stdClass();
            $temp_object->id=$ticketing_vip_page->serial_id;
            $temp_object->first_name=$ticketing_vip_page->first_name;
            $temp_object->last_name=$ticketing_vip_page->last_name;
