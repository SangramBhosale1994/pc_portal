...
// echo "***********";
	...
	if(isset($_POST['order'])){
		...
		foreach($post_values as $key=>$value){
			if($key=="order"){
				...
				if($order_column!=0){
					// $requested_columns_to_show_sort_order=$post_colums[$order_column*1];
	...
	$exception_search_fields=["checkbox","unlock_button","serial_number","registration_time","recruiter_company_name","last_modified_date","job_publish_shedule_for_table","application_deadline_for_table","hot_jobs","is_show_popup","applicants_list_button","edit","job_preview"];
	...
	// }
		...
		$loop_counter=1;
		 ...
		 $all_jobs_array=\ProcessWire\wire('pages')->get("name=jobs")->children($filter_selector);
		// $all_jobs_array=\ProcessWire\wire('pages')->get("name=jobs")->children("sort=-company_name");
	    ...
	    $to_return->job_title=$job_data->title;	
			...
			if($job_page->is_popup_show_event!=""){
				// $to_return->is_popup_show_event ="111";
				$is_show_popup = $job_page->is_popup_show_event;
				if($is_show_popup=="show"){
					$job_data->is_show_popup = "Show";
				}
				elseif($is_show_popup=="hide"){
					$job_data->is_show_popup = "Hide";
				}
				else{
					$job_data->is_show_popup = "Hide";
				}
			...
			// $to_return->is_show_popup = $job_data->is_show_popup ;
...
// 			$job_data->job_publish_shedule_for_table = date("d M Y", $job_page->job_publish_shedule);
			...
			$applicants_list_link=\ProcessWire\wire('pages')->get("name=admin-candidate-table")->httpUrl."?job_code=".$job_data->job_code;
			...
			if($job_page->job_added_by !=0){
				...
				$admin_page=\ProcessWire\wire('pages')->get("name=admins")->child("id=".$job_page->job_added_by);
				//echo $admin_page->company_name;
				$editor_page=\ProcessWire\wire('pages')->get("name=editors")->child("id=".$job_page->job_added_by);
				$recruiter_page=\ProcessWire\wire('pages')->get("name=recruiters")->child("id=".$job_page->job_added_by);
				//echo $recruiter_page;
				...
				elseif($recruiter_page->id!=0){
					$job_data->recruiter_company_name=$recruiter_page->company_name;
				}
		...
		$to_return->columns = [
			...
			["title"=>"Popup Show Status", "data"=>"is_show_popup"],
...
// 		$new_job_page->job_added_by = \ProcessWire\wire('session')->user_page_id;
		
// 		/* Define the temporary path to upload the file before saving the files to the CMS page */
// 		$upload_path = \ProcessWire\wire('config')->paths->assets;//. "files/temp-files/";
...
// 	}
	...
	function unlock_profiles($requested_job_profile_to_unlock){
		...
		$job_page = \ProcessWire\wire("pages")->get("id=".$requested_job_profile_to_unlock);
		$job_page->of(false);
	...
	function unverify_profiles($requested_job_profile_to_unverify_profiles){
		...
		/* Get the page of the editor */
		$job_page = \ProcessWire\wire("pages")->get("id=".$requested_job_profile_to_unverify_profiles);
		$job_page->of(false);
//echo "1";
...
// echo "***********";
		...
		foreach($requested_profiles_to_verify as $single_job_page){
			...
			$job_page=\ProcessWire\wire("pages")->get("id=".$single_job_page);
			//echo $job_page->id;
