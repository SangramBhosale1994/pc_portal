...
// 	 print_r($company_admin_profile);
		...
		$serial_counter=1;
		foreach(\ProcessWire\wire('pages')->get("name=referral-profiles")->children("sort=-published") as $referral_page) {
		  ...
		  //  foreach(\ProcessWire\wire('pages')->get("name=universal-profiles")->children("company_page_id=".$referral_page->id) as $referral_admin_profile){
		  
		  $referral_admin_profile = \ProcessWire\wire('pages')->get("name=universal-profiles")->child("referrer_page_id=".$referral_page->id);
...
// 		      $referral_admin_profile = \ProcessWire\wire('pages')->get("name=universal-profiles")->child("email=".$referrer_email);
...
// 		  }
      ...
      // echo "******";                  
			...
			if($referral_admin_profile->identify_as!="Ally"){
				...
				$candidate_emails=\ProcessWire\wire('pages')->get("name=candidates")->child("email=".$single_referre_email);
...
// 			["title"=>"Employees", "data"=>"employee_count"],
	...
	function unlock_profiles($requested_referral_profile_to_unlock){
		...
		$referrer_page = \ProcessWire\wire("pages")->get("name=referral-profiles")->child("id=".$requested_referral_profile_to_unlock);
...
// 		if($input->post->email){
    	 ...
    	 //echo $modal_referrer_id;
		    ...
		    //$referrer_page = \ProcessWire\wire('pages')->get("name=referral-profiles")->child("id=".$edit_profile_form_details);
    	//	 $referral_page= \ProcessWire\wire('pages')->get("name=referral-profiles")->child('title='.$company_name);
    		 
    		 //$candidate_page_id= \ProcessWire\wire('pages')->get("name=candidates")->children();
    		 
    		 $referrer_page = \ProcessWire\wire('pages')->get("name=referral-profiles")->child("id=".$modal_referrer_id);
    		// echo $referrer_page;
...
//echo "**************************************";
      ...
      $referrer_page->save();
            ...
            $link =\ProcessWire\wire('pages')->get("name=universe")->httpUrl;
...
// 		}
	...
	// function add_points_multiple_profiles($add_points_to_multiple_profiles){
		...
		foreach($add_points_to_multiple_profiles as $single_referral_id){
			$referrer_page = \ProcessWire\wire('pages')->get("name=referral-profiles")->child("id=".$single_referral_id);
			/* Array to save data and then to save into the page END */
