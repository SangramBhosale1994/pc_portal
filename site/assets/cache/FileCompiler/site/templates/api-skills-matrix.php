...
// print_r($_POST);
	...
	function fetch_skills_request_table_data(){
		...
		/* Code Amruta Jagtap*/	
    		...
    		// foreach(\ProcessWire\wire('pages')->get("name=candidates")->children("limit=20,".$filter_selector) as $candidate_page) {
						...
						if(!empty($search_skills_array)){
							foreach($search_skills_array as $skills){
								...
								// $skills_slug=\ProcessWire\wire('sanitizer')->pageName($skills, true);
								$skill_object->count=\ProcessWire\wire('pages')->get("name=candidates")->children("skills|soft_skills|technical_skills|non_technical_skills%=".$skills.",".$filter_selector)->count();
										// echo \ProcessWire\wire('pages')->get("name=candidates")->children("skills%=".$skills.",".$filter_selector)->count();
							...
							foreach($candidate_skills_array as $skill){
								...
								// $skills_slug=\ProcessWire\wire('sanitizer')->pageName($skill, true);
								$skill_object->count=\ProcessWire\wire('pages')->get("name=candidates")->children("skills|soft_skills|technical_skills|non_technical_skills%=".$skill)->count();
								array_push($array_to_send,$skill_object);
								// print_r($array_to_send);
							}
						}
        ...
        /**End Skills data using candidate profile */   
    		...
    		$to_return->columns = [
				...
				// 			$skills_slug=\ProcessWire\wire('sanitizer')->pageName($single_skills, true);
				// 				$to_return->columns[]=
	...
	function fetch_skills_location_request_table_data(){
		...
		/* Code Sameesh Yadav*/	
    		...
    		$skills_array = array();
				...
				// foreach(\ProcessWire\wire('pages')->get("name=candidates")->children("active_status=active|Active,current_city!=,skills!=") as $candidate_page) {
			...
			$all_locations_array = \ProcessWire\wire('pages')->get("name=candidates")->children()->each('current_city'); // get the titles of subjects (pages)foreach ($subjects as $subject)
			...
			// print_r($skills_array);
				...
				foreach($skills_array as $single_skill){
					...
					foreach($location_array_count as $single_location){
						...
						$location_slug=\ProcessWire\wire('sanitizer')->pageName($single_location, true);
						if($location_slug==""){
							continue;
						}
					...
					if($single_skill=="total"){
						$single_skill_current_city_array=\ProcessWire\wire('pages')->get("name=candidates")->children()->each("current_city");
						// 	print_r($single_skill_current_city_array);
						// echo "****"; 
					}
					else{
						$single_skill_current_city_array=\ProcessWire\wire('pages')->get("name=candidates")->children("skills|soft_skills|technical_skills|non_technical_skills%=".$single_skill)->each("current_city");
					}
				...
				// echo "****"; 
					...
					foreach($single_skill_location_array_count as $single_location_name=>$single_location_count){
						$location_slug=\ProcessWire\wire('sanitizer')->pageName($single_location_name, true);
						if($location_slug==""){
							continue;
						}
					...
					// 	$location_slug=\ProcessWire\wire('sanitizer')->pageName($single_location_key, true);
					// 	// echo "-----";
					// 	// echo $single_location_key;
					// 	// $candidate_count[]=\ProcessWire\wire('pages')->get("name=candidates")->children("skills%=".$single_skill.",current_city=".$single_location)->count();
					// 	$skill_object->$location_slug=\ProcessWire\wire('pages')->get("name=candidates")->children("skills%=".$single_skill.",current_city=".$single_location_key)->count();
	...
	function fetch_skills_identify_as_request_table_data(){
		...
		global $to_return;
          ...
          //  echo "*****"; 
    		// foreach(\ProcessWire\wire('pages')->get("name=candidates")->children("active_status=active|Active,skills!=,identify_as!=") as $candidate_page) {
				...
				foreach($skills_array as $single_skill){
					...
					// if(!empty($candidate_identify_as_array)){
						...
						foreach($candidate_identify_as_array as $single_identify_as){
							$identify_as_slug=\ProcessWire\wire('sanitizer')->pageName($single_identify_as, true);
						// 	echo "222****";
						// echo $single_identify_as;
						// echo "3333----";
						// echo \ProcessWire\wire('pages')->get("name=candidates")->children("skills%=".$single_skill.",identify_as%=".$single_identify_as)->count();
							// $candidate_count[]=\ProcessWire\wire('pages')->get("name=candidates")->children("skills%=".$single_skill.",identify_as%=".$single_identify_as)->count();
							
							// $skill_object->$identify_as_slug=\ProcessWire\wire('pages')->get("name=candidates")->children("skills|soft_skills|technical_skills|non_technical_skills%=".$single_skill.",identify_as%=".$single_identify_as)->count();
							if($single_skill=="total"){
								$skill_object->$identify_as_slug=\ProcessWire\wire('pages')->get("name=candidates")->children("identify_as=".$single_identify_as)->count();
							}
							else{
								$skill_object->$identify_as_slug=\ProcessWire\wire('pages')->get("name=candidates")->children("skills|soft_skills|technical_skills|non_technical_skills%=".$single_skill.",identify_as=".$single_identify_as)->count();
							}
				...
				foreach($candidate_skills_array as $single_skill){
					...
					foreach($candidate_experience_array as $single_experience_object){
						...
						$experience_implode_data=implode("|",$experience_array_for_implode);
							...
							// 	echo "----";
			 			// $experience_implode_data=implode("|",$experience_array);
						$experience_slug=\ProcessWire\wire('sanitizer')->pageName($single_experience_object, true);
						
						// echo "experience_years<=".$experience_implode_data.",experience_years>=".$experience_implode_data;
						if($single_skill=="total"){
							$skill_object->$experience_slug=\ProcessWire\wire('pages')->get("name=candidates")->children("experience_years=".$experience_implode_data)->count();
						}
						else{
							$skill_object->$experience_slug=\ProcessWire\wire('pages')->get("name=candidates")->children("skills|soft_skills|technical_skills|non_technical_skills%=".$single_skill.",experience_years~=".$experience_implode_data)->count();
						}
