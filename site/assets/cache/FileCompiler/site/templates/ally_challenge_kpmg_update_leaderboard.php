...
error_reporting(E_ALL);
	...
	/* Live credentials */
	// $dbHost = 'localhost'; 
	// $dbName = 'admin_resume_db';
	// $dbUser = 'admin_resume_db';
	// $dbPass = 'PrideCircle@456';
...
//echo "h1";
		...
		$check_organisation_query = mysqli_query( $db_connection, "SELECT * FROM kpmg_ally_challenge_scores WHERE contestant_email = '". $organisation_email."' LIMIT 1");
...
//echo "h3";
				...
				$entry_query = mysqli_query($db_connection, "UPDATE `kpmg_ally_challenge_scores` SET `company_name`='$organisation_title',`reference_email`='' WHERE contestant_email='$organisation_email'");
//print_r($entry_query);
//echo "asd";
			}
...
//echo "continued";
		...
		$contestant_country = mysqli_real_escape_string($db_connection, $entry_info[14]);
		...
		/* try to fetch account with given email */
		$check_account_query = mysqli_query( $db_connection, "SELECT * FROM kpmg_ally_challenge_scores WHERE contestant_email = '$contestant_email' LIMIT 1");
		...
		if(mysqli_num_rows($check_account_query) === 1){
			...
			if($account_info["profile_status"] === "incomplete"){
				...
				mysqli_query($db_connection, "UPDATE `kpmg_ally_challenge_scores` SET `contestant_name`='$contestant_name',`contestant_country`='$contestant_country',`profile_status`= 'complete' WHERE contestant_email='$contestant_email'");
			}
		...
		mysqli_query($db_connection, "UPDATE `kpmg_ally_challenge_scores` SET `contestant_score`=contestant_score+$entry_score WHERE contestant_email = '$contestant_email' LIMIT 1");
		...
		if($is_group){
			...
			if($entry_action === "Refer Folx & Earn Bonus Entries (Points) #WinTogether"){
				...
				$check_referee_account_query = mysqli_query($db_connection, "SELECT * FROM `kpmg_ally_challenge_scores` WHERE contestant_email = '$reference_email' LIMIT 1");
				...
				if(mysqli_num_rows($check_referee_account_query) === 1){
					...
					if($referee_account_info['reference_email']==""){
						...
						mysqli_query($db_connection, "UPDATE `kpmg_ally_challenge_scores` SET `reference_email`='$contestant_email' WHERE contestant_email='$reference_email'");
					...
					if($add_referree_score){
						...
						mysqli_query($db_connection, "UPDATE `kpmg_ally_challenge_scores` SET `contestant_score`=contestant_score+$referee_score WHERE contestant_email='$contestant_email'");
					}
		...
		/* If it is an individual */
			...
			$check_for_group_referrer_query = mysqli_query($db_connection, "SELECT * FROM `kpmg_ally_challenge_scores` WHERE contestant_email = '$contestant_email' AND reference_email!='' LIMIT 1");
			...
			if(mysqli_num_rows($check_for_group_referrer_query) === 1){
				...
				mysqli_query($db_connection, "UPDATE `kpmg_ally_challenge_scores` SET `contestant_score`=contestant_score+$entry_score WHERE contestant_email = '$group_referrer_email' LIMIT 1");
			}
	...
	$group_leaderboard_result = mysqli_query($db_connection, "SELECT * FROM kpmg_ally_challenge_scores WHERE contestant_country = 'KGS' AND contestant_score>0 ORDER BY contestant_score DESC LIMIT 50");

	if($group_leaderboard_result){
		while($group_leader = mysqli_fetch_assoc($group_leaderboard_result)){
			$group_ally_count_result = mysqli_query($db_connection, "SELECT reference_email, COUNT(*) AS ally_count FROM kpmg_ally_challenge_scores WHERE reference_email='".$group_leader['contestant_email']."'");
	...
	$individual_leaderboard_result = mysqli_query($db_connection, "SELECT * FROM kpmg_ally_challenge_scores WHERE contestant_country = 'KI' ORDER BY contestant_score DESC LIMIT 50");
	...
	/* 2021-10-06 Servicenow eight scoreboards fetching */
	/* An array of all the leaderboard countries there are going to be (apart from the general individual leaderboard) */
	$all_countries_array = Array("Australia","New Zealand","Hong Kong","Singapore","Japan","India","South Korea");

	$all_countries_leaderboard_array = Array();

	/* Loop through all the countries to get thier scores */
	foreach($all_countries_array as $single_country){
		/* Array to save scores of a single country */
		$single_country_leaderboard_array = Array();

		$single_country_leaderboard_result = mysqli_query($db_connection, "SELECT * FROM kpmg_ally_challenge_scores WHERE contestant_country = '".$single_country."' AND contestant_score>0 ORDER BY contestant_score DESC  LIMIT 10");

		if($single_country_leaderboard_result){
			while($single_country_leader = mysqli_fetch_assoc($single_country_leaderboard_result)){
				$single_country_ally_count_result = mysqli_query($db_connection, "SELECT reference_email, COUNT(*) AS ally_count FROM kpmg_ally_challenge_scores WHERE reference_email='".$single_country_leader['contestant_email']."'");

				$single_country_leader["ally_count"] = mysqli_fetch_assoc($single_country_ally_count_result)["ally_count"];
				$single_country_leaderboard_array[] = $single_country_leader;
			}
		}
		else{
			echo "ERROR!1";
		...
		/* Add single country array to the all countries array. */
		$all_countries_leaderboard_array[$single_country] = $single_country_leaderboard_array;
	}
	/* Loop through all the countries to get thier scores END */

	/* INDIVIDIAL Leaderboard */
	$individual_leaderboard_array = Array();

	$individual_leaderboard_result = mysqli_query($db_connection, "SELECT * FROM kpmg_ally_challenge_scores WHERE contestant_score>0 ORDER BY contestant_score DESC LIMIT 10");

	if($individual_leaderboard_result){
		while($individual_leader = mysqli_fetch_assoc($individual_leaderboard_result)){
			$individual_ally_count_result = mysqli_query($db_connection, "SELECT reference_email, COUNT(*) AS ally_count FROM kpmg_ally_challenge_scores WHERE reference_email='".$individual_leader['contestant_email']."'");

			$individual_leader["ally_count"] = mysqli_fetch_assoc($individual_ally_count_result)["ally_count"];
			$individual_leaderboard_array[] = $individual_leader;
		}
	}
	else{
	...
	/* INDIVIDIAL Leaderboard END */

	/* Add individual to the all leaderboards array */
	$all_countries_leaderboard_array['individual'] = $individual_leaderboard_array;

	/* 2021-10-06 Servicenow eight scoreboards fetching END */
	...
	$page->ally_challenge_leaderboard_group = json_encode($all_countries_leaderboard_array);
...
//echo $dataArray_json;
	...
	function add_contestant($db_connection, $contestant_email, $entry_score, $reference_email, $contestant_name, $contestant_country, $profile_status){
		$add_contestant_result = mysqli_query($db_connection, "INSERT INTO kpmg_ally_challenge_scores (`contestant_email`, `contestant_score`, `reference_email`, `contestant_name`, `contestant_country`,`company_name`, `profile_status`) VALUES ('$contestant_email',$entry_score,'$reference_email','$contestant_name','$contestant_country','', '$profile_status')");
...
echo "ERROR!3";
	...
	function csvstring_to_array($string, $separatorChar = ',', $enclosureChar = '"', $newlineChar = "\n") {
	    ...
	    0 "Name"
			...
			7	"Details"
			11	"When"
			14/16/18/20	"Country (User Details)"
		*/
	    $needed_columns_array = [0, 3, 4, 5, 6, 7, 11, 14];
...
//echo $char;
	...
	$group_leaderboard_result = mysqli_query($db_connection, "SELECT * FROM kpmg_ally_challenge_scores WHERE contestant_country = 'KGS' AND contestant_score>0 ORDER BY contestant_score DESC");
	...
	/* Fetch the Individual Leaderboard */
	$individual_leaderboard_array = Array();
	$individual_leaderboard_result = mysqli_query($db_connection, "SELECT * FROM kpmg_ally_challenge_scores WHERE contestant_country = 'KI' ORDER BY contestant_score DESC");
	...
	/* 2021-10-06 Servicenow eight scoreboards fetching */
		...
		foreach($all_countries_array as $single_country){
			...
			$single_country_leaderboard_result = mysqli_query($db_connection, "SELECT * FROM kpmg_ally_challenge_scores WHERE contestant_country = '".$single_country."' AND contestant_score>0 ORDER BY contestant_score DESC");
 ...
 <body>
 	...
 	<?php
 		foreach ($all_countries_leaderboard_array as $single_country_title => $single_country_leaderboard) {
 	?>
 	...
 	<div>
		<center>
			<table>
				<caption><?=$single_country_title?> Leaderboard</caption>
				<thead>
					<tr>
						<th>Rank</th>
						<th>Name</th>
				...
				<tbody>
					<?php
							...
							foreach($single_country_leaderboard as $individual_leader){
								$counter++;
					?>
					...
					<tr>
						...
						<td><?=$individual_leader["contestant_email"] ?></td>
 	...
 	<div>
		...
		<center>
			...
			<?php
 			...
 			$all_companies_emails = mysqli_query( $db_connection, "SELECT company_name, contestant_email FROM kpmg_ally_challenge_scores WHERE company_name !=''");	
 			...
 			<?php
 				$company_referral_emails = mysqli_query( $db_connection, "SELECT * FROM kpmg_ally_challenge_scores WHERE reference_email ='".$main_row['contestant_email']."' ORDER BY contestant_score DESC LIMIT 10");
