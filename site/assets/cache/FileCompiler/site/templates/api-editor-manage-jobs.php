...
// echo "***********";
	...
	// }
		...
		if(isset($_GET['job_added_by'])){
			...
			$recruiter_page = \ProcessWire\wire("pages")->get("id=".$job_added_by);
			$recruiter_page_array[].=$recruiter_page->id;
			foreach(\ProcessWire\wire("pages")->get("id=".$job_added_by)->children("recruiter_profile_type=sub-recruiter") as $sub_recruiter_page){
...
// 			$job_data->job_publish_shedule_for_table = date("d M Y", $job_page->job_publish_shedule);
			...
			if($job_page->job_added_by !=0){
				...
				$recruiter_page=\ProcessWire\wire('pages')->get("id=".$job_page->job_added_by);
				//echo $recruiter_page;
