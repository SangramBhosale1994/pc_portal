...
//  function expired_jobs(\ProcessWire\HookEvent $e){
function expired_jobs(){
  $current_timestamp=strtotime(date("Y-m-d h:i:sa"));
  //$counter=0;
  // foreach(\ProcessWire\wire('pages')->get("name=jobs")->children("application_deadline<".$current_timestamp) as $expired_job_page) {
    foreach(\ProcessWire\wire('pages')->get("name=jobs")->children("sort=-published,verification=unverified,limit=5") as $expired_job_page) {
...
expired_jobs();
// \ProcessWire\wire()->addHook('LazyCron::every2Minutes', null, 'expired_jobs');
?>