...
//$universal_profiles_page=\ProcessWire\wire('pages')->get('name=universal-profiles');
	$serial_counter=1;   
		foreach(\ProcessWire\wire('pages')->get("name=pass-applicants")->children("sort=published") as $job_fair_page) {
			//$company_page_title=$sanitizer->selectorValue($company_page->title);
		    $candidate_job_fair_verified_profile = \ProcessWire\wire('pages')->get("name=candidates")->child("id=".$job_fair_page->candidate_profile_id);
		    
		     $candidate_job_fair_unverified_profile = \ProcessWire\wire('pages')->get("name=candidates")->child("id=".$job_fair_page->candidate_profile_id);
		  //   echo "************";
		  //   echo $candidate_lgbt_unverified_profile;
		     
		    $candidate_job_fair_profile = \ProcessWire\wire('pages')->get("name=candidates")->child("id=".$job_fair_page->candidate_profile_id);
    ...
    // 			$job_fair_data->verify_button = "<button id='".$job_fair_page->id."_verify_button' class='[ btn btn-primary ]( verify_button )' type='button'><i class='[ mr-2 ][ fas fa-unlock ]'></i>Verify</button>";
		...
		//    }
		   ...
		   if($candidate_job_fair_profile->id!=0 && $job_fair_page->pass_verification=="" ){
				// $serial_counter=1;
						/* Each candidate's data is to be saved in a seperate object */
						$candidate_page=\ProcessWire\wire('pages')->get("name=candidates")->child("id=".$job_fair_page->candidate_profile_id);
						$job_fair_data = new stdClass();
						...
						//echo $candidate_lgbt_unverified_profile  http://zerovaega.in/pc_rportal/resume/edit-application/;
						      ...
						      $candidate_edit_application=\ProcessWire\wire('pages')->get("name=edit-application")->httpUrl;
	...
	function verify_profiles($requested_job_fair_to_verify){
		...
		$job_fair_page = \ProcessWire\wire("pages")->get("name=pass-applicants")->child("id=".$requested_job_fair_to_verify);
...
//echo"5";
	...
	function unverify_profiles($requested_job_fair_to_unverify_profiles){
		...
		/* Get the page of the editor */
		$job_fair_page = \ProcessWire\wire("pages")->get("id=".$requested_job_fair_to_unverify_profiles);
		$job_fair_page->of(false);
//echo "1";
...
//echo "3";
    ...
    // echo "***********";
		...
		foreach($requested_profiles_to_unverify as $single_applicant_page){
			...
			$applicant_page=\ProcessWire\wire("pages")->get("id=".$single_applicant_page);
